...
# Usage: rake postmd title="A Title"  date="20120209" tags=[tag1,tag2]
desc "Begin a new postmd in #{CONFIG['posts']}"
task :postmd do
  ...
  ## 这个可以直接显示中文名称:
  # filename = File.join(CONFIG['posts'], "#{date}-#{title}.md")
  filename = File.join(CONFIG['posts'], "#{date}-#{title.gsub('：','-').gsub(' ','-').gsub(':','-')}.md")
  ...
  open(filename, 'w') do |post|
    ...
    # ----------------------------------------------
    post.puts "title: \"#{title.gsub(/-/,' ')}\""
    post.puts "author: William"
    post.puts "date: #{date}"  
    post.puts "lastmod: #{date}" 
...
# Usage: rake post title="A Title"  date=20120209 tags=[tag1,tag2]
...
task :post do
  ...
  ## 这个可以直接显示中文名称:
  # filename = File.join(CONFIG['posts'], "#{date}-#{title}.Rmd")
  filename = File.join(CONFIG['posts'], "#{date}-#{title.gsub('：','-').gsub(' ','-').gsub(':','-')}.Rmd")
  ...
  open(filename, 'w') do |post|
    ...
    post.puts "---"
    post.puts ""
    post.puts "```{r echo=FALSE}"
    post.puts "knitr::opts_chunk$set(comment = '',"
    post.puts "                      fig.align = 'center',"
